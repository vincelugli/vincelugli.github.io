...
describe("executeAutoPick Cloud Function", () => {
  ...
  it(
    "should draft from priority list even if the role is already filled",
    async () => {
    // ARRANGE
  ...
  it(
    "should recalculate future round pick orders at the end of " +
      "a round based on team Elo sums",
    async () => {
    // ARRANGE
      ...
      // Two teams:
      // Team 1 starts with Viper (id 1, Diamond 1, ELO 79) +
      // Kilo (id 9, Bronze 1, ELO 29) = ELO sum 108
      // Team 2 starts with Ghost (id 2, Emerald 1, ELO 69) and
      // is about to pick.
      const team1 = {
        id: 1,
        name: "Team Viper",
      ...
      // Raptor (id 5, Plat 1, ELO 59), Blaze (id 6, Gold 1, ELO 49),
      // Omega (id 8, Silver 1, ELO 39)
      const available = allPlayers.filter((p) => ![1, 2, 9].includes(p.id));

      // Initially: round 1 has index 0 and 1.
      // We are at pick index 1 (end of round 1).
      // Initial snake prediction for round 2 was:
      // index 2 is Team 2, index 3 is Team 1.
      ...
      // Team 2 priority list is empty, so it will draft
      // the highest Elo: Raptor (id 5, ELO 59).
      // Team 2 ELO sum will be Ghost (69) + Raptor (59) = 128.
      // Since Team 1 has ELO sum 108 (lower than 128),
      // Team 1 should pick first in Round 2!
      // So pick order for Round 2 should become Team 1 then Team 2: [1, 2].
      // Let's configure mock databases
      mockGet.mockResolvedValueOnce({exists: true, data: () => draftState});
      // No priority list for Team 2
      ...
      // Round 2 picks (indices 2 and 3) should be updated to
      // [1, 2] instead of [2, 1]
      // Round 3 picks (indices 4 and 5) should alternate
      // reversed: [2, 1] instead of [1, 2]
      // Round 4 picks (indices 6 and 7) should alternate
      // standard: [1, 2] instead of [2, 1]
      // Round 5 picks (indices 8 and 9) should alternate
      // reversed: [2, 1] instead of [1, 2]
      expect(updateCallPayload.pickOrder).toEqual([
        1, 2, 1, 2, 2, 1, 1, 2, 2, 1,
      ]);
    });
});
